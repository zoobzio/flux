...
The final processing stage in a Capacitor pipeline.
...
Creates a rate limiting processor. Uses a token bucket algorithm with the specified rate (tokens per second) and burst size. When tokens are exhausted, requests wait for availability.
